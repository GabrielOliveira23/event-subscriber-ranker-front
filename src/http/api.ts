...
export type GetSubcriberInviteCount200 = {
  count: number
}
...
export const getSubscribeToEventUrl = () => {
  return 'http://localhost:4000/subscriptions'
}
...
export const getAccessInviteLinkUrl = (subscriberId: string) => {
  return `http://localhost:4000/invites/${subscriberId}`
}
...
export const getGetSubscriberInviteClicksUrl = (subscriberId: string) => {
  return `http://localhost:4000/subscribers/${subscriberId}/ranking/clicks`
}
 ...
 */
export const getGetSubcriberInviteCountUrl = (subscriberId: string) => {
  return `http://localhost:4000/subscribers/${subscriberId}/ranking/count`
}

export const getSubcriberInviteCount = async (
  subscriberId: string,
  options?: RequestInit
): Promise<GetSubcriberInviteCount200> => {
  const res = await fetch(getGetSubcriberInviteCountUrl(subscriberId), {
    ...options,
    method: 'GET',
  })

  const body = [204, 205, 304].includes(res.status) ? null : await res.text()
  const data: GetSubcriberInviteCount200 = body ? JSON.parse(body) : {}
...
export const getGetSubscriberRankingPositionUrl = (subscriberId: string) => {
  return `http://localhost:4000/subscribers/${subscriberId}/ranking/position`
}
...
export const getGetRankingUrl = () => {
  return 'http://localhost:4000/ranking'
}
